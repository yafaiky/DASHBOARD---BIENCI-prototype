...
import { Plus, Shield, CheckCircle2, MoreVertical } from "lucide-react"
...
export const UsersPage: React.FC = () => {
  const [users] = useState([
    ...
    },
    {
      ...
      role: "Viewer (Koordinator Klaster Toko)",
      status: "Aktif",
      lastLogin: "20 Okt 2026",
      created: "05 Jun 2025",
    },
  ])
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">
            Pengguna Sistem & Manajemen Hak Akses
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Pengaturan akun administrator HR, peninjau eksekutif, dan analis toko regional
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => alert("Dialog Undang Pengguna Baru")}
          ...
          <table className="w-full text-xs text-left">
            <thead className="bg-slate-50 text-slate-500 uppercase text-[10px] tracking-wider border-b border-slate-100 font-bold">
              <tr>
                ...
                <th className="p-3">Akun Dibuat</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {users.map((u) => (
                <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-3">
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-lg text-[10px] font-bold bg-slate-100 text-slate-800">
